(ns hashicorp.main
  "Kotodama WASM entrypoint for the Hashicorp clean-room actor (L4) — Clojure port.
  ...
  facts (`hashicorp.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "hashicorp")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"hashicorp.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "hashicorp-compat" :tier tier :entities entities} 200])
...
(defn start! [] :hashicorp-compat/ready)
